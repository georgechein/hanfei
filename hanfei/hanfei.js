...
Hanfei.prototype.CreateNormal = function (text_msg) {
    nodejieba.load({
        userDict: './dist/QA.utf8',
    });
    ...
    var tsti_results;
    try {
        cut_results = nodejieba.tag(text_msg);
        tsti_results = cut_results.filter(function (val) {
            if (val.tag == "tsti") return val;
        });
        undef_results = cut_results.filter(function (val) {
            if (val.tag == "x") return val;
        });
    } catch (e) {
        logger.error("User input: " + text_msg + "\nJieba_error: " + e.stack);
        var system_error = '不好意思，目前系統忙碌中，請稍後在試';
        return system_error;
    }
    
    logger.info("recard time");
    
    if (undef_results.length > 0) {
        try {
            setUndefWords(undef_results);
        } catch (e) {
            logger.error("User input: " + text_msg + "\nRecords_word_error: " + e.stack);
        }
    }

    if (tsti_results.length > 0) {
        var tsti_return = new Phrase();
        tsti_return.qna = tsti_results[0].word;
        tsti_return.line = tsti_results[0].word;
        return tsti_return;
    }
    ...
    try {
        ...
        logger.error("User input: " + text_msg + "\nShowMessage error: " + e.stack);
        var system_error = '不好意思，目前系統忙碌中，請稍後在試';
        return system_error;
    }

    var phrase_return = new Phrase();
    phrase_return.qna = phrase_obj.intend;
    phrase_return.line = phrase_msg;
...
/**
 * 如果結巴判斷字典內無此類型
 * 則存入dict/Missing.json
 */
function setUndefWords(undef_words) {
    var undef_list = [];
    getCsv(__dirname + "/dist/undef_list.csv", function (csv_file) {
        undef_list = csv_file;
        undef_words.forEach(function (undef_word) {
            var w_has = 0;
            var w_pos;

            for (var index = 0; index < undef_list.length; index++) {
                if (undef_list[index]["word"] != undefined && undef_list[index]["word"].indexOf(undef_word.word) != -1) {
                    w_has = 1;
                    w_pos = index;
                    break;
                }
            ...
            if (w_has == 1) {
                undef_list[w_pos].weight++;
            } else {
                undef_list.push({ "word": undef_word.word, "weight": 1, "tag": "x" });
            }

        });
        undef_list.sort(UP);
        setCsv(__dirname + "/dist/undef_list.csv", undef_list);
    });
}
...
function getCsv(path, callback) {
    ...
    csv.fromPath(path, { headers: true })
        ...
        .on("end", function () {
        logger.info("csv 讀取成功!");
        callback(file);
    })
        .on("error", function (e) {
        logger.error("csv 讀取失敗， 失敗原因: " + e);
        var error_file = [];
        callback(error_file);
    });
}
...
//寫入csv檔案
function setCsv(path, data) {
    var csv = require("fast-csv");
    csv.writeToPath(path, data, { headers: true })
        .on("finish", function () {
        logger.info("csv 寫入成功!");
    })
        .on("error", function (e) {
        logger.error("csv 寫入失敗， 失敗原因: " + e);
    });
}
...
function getSentenceStructure(word_group, phrase, sentence) {
    ...
    for (i = 0; i < word_group.length; i++) {
        ...
        logger.info('Phrase Words: ' + phrase_obj.words[phrase_index + word_index].toString() + ' Phrase tags: ' + phrase_obj.tags[phrase_index + word_index].toString());
    ...
    for (i = 0; i < phrase_obj.count; i++) {
        ...
        logger.info('Sentence words: ' + phrase_obj.words[i].toString());
...
function dividePhraseByPron(word_pos, index, phrase_obj) {
    ...
    logger.info('Pron Words: ' + words.toString() + ' Pron Tags: ' + tags.toString());
...
function dividePhraseByUj(uj_pos, word_count, phrase_index, phrase_obj) {
    ...
    logger.info("包含'的'的短語: " + words.toString() + " 包含'的'的詞性: " + tags.toString());
    // V + 的 + O - 謂語
    if (uj_pos == 0) {
        phrase_obj.words[phrase_index] = [];
        phrase_obj.tags[phrase_index] = [];
        ...
        if (uj_pos + 2 < words.length && uj_pos - 1 > 0) {
            ...
            // r + N + 的 + N 短語
        } else if (uj_pos + 2 >= words.length && uj_pos - 1 == 0) {
            pos = [0, words.length];
            // N + 的 + N 短語末端
...
function dividePhrase(pos, words, tags, phrase_obj) {
    ...
    for (i = 0; i < pos.length - 1; i++) {
        ...
        logger.debug("Each Dividing Word: " + words.slice(pre, pro).toString());
        phrase_obj.tags.push(tags.slice(pre, pro));
        logger.debug("Each Dividing Tag: " + tags.slice(pre, pro).toString());
    }
    logger.debug("Dividing Phrase Words: " + phrase_obj.words.toString() + 'Dividing Phrase Tags: ' + phrase_obj.tags.toString());
}
...
function getIntendByGrammar(phrase_obj, cut_results) {
    ...
    logger.debug('Sentence Grammar type: ' + phrase_obj.sentence_type);
    logger.debug('Before Grammar intend: ' + phrase_obj.intend);
    ...
    logger.debug('After Grammar intend: ' + phrase_obj.intend);
    return intend;
}
...
function getCommonIntend(cut_results) {
    ...
    for (i = 0; i < cut_results.length; i++) {
        ...
        switch (cut_results[i].tag) {
            ...
            // 時間
            case 't':
                t_msg = cut_results[i].word;
                break;
            default:
                n_msgs = n_msgs + cut_results[i].word;
        }
        logger.info('分詞結果 : ' + JSON.stringify(cut_results[i]));
        logger.debug('Common intend v_msg: ' + v_msg);
        logger.debug('Common intend n_msgs: ' + n_msgs);
        logger.debug('Common intend t_msg: ' + t_msg);
    }
    ret_msg = t_msg + v_msg + n_msgs;
